...
public void OnPluginStart()
{
	...
	g_hConsumeRockInterval = CreateConVar("ai_TankConsumeRockInterval", "3", "坦克在消耗位上时多少秒扔一次石头", CVAR_FLAG, true, 0.0);
	// 目标选择
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	if (IsAiTank(client))
	{
		...
		if (IsValidSurvivor(target))
		{
			...
			if (!eTankStructure[client].bCanConsume && eTankStructure[client].fTankStopDistance <= targetdist <= 1000 && curspeed > 190.0)
			{
		...
		if (!eTankStructure[client].bCanConsume && GetGameTime() - eTankStructure[client].fRockThrowTime < ROCK_AIM_TIME && !IsOnLadder(client))
		{
